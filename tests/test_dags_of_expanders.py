...
def make_input(instance: ConstructedInstance):
    ...
    input = instance.export_russian_graph()

    return input, h, w


INPUT_EXPECTED = [
    pytest.param(ConstructedInstance.new(
        seed=420,
        num_ranks=3,
    ...
    pytest.param(ConstructedInstance.new(
        ...
        expander_degree=4,
    ), 66, id="10size_5x5_expanders"),
]


@pytest.mark.expander
@pytest.mark.parametrize("instance,expected", INPUT_EXPECTED)
def test_flow_with_dag_of_expanders_without_weight_fn(instance: ConstructedInstance, expected: int):
...
@pytest.mark.expander
@pytest.mark.parametrize("instance,expected", INPUT_EXPECTED)
def test_flow_with_dag_of_expanders_with_weight_fn(instance: ConstructedInstance, expected: int):
